...
**⚠️ 必须步骤：** 查询到原始数据后，不要直接生成报告。先进行深度分析，挖掘数据背后的洞察和归因，让报告具有真正的分析价值。
...
- **核心指标计算**：
  - 总量、均值、中位数、众数、标准差
  - 最大值/最小值、极差、四分位距
  - 增长率（同比 YoY、环比 MoM/QoQ）
  - 占比（各分类在整体中的比例）

- **数据分布特征**：
  - 集中趋势：数据集中在什么水平？
  - 离散程度：数据波动是否剧烈？（用标准差/变异系数衡量）
  - 偏态分析：数据是否存在明显的头部或尾部效应？
...
#### 4.2 趋势分析（How is it changing）

识别数据的时间模式和变化方向：

- **趋势方向**：上升、下降、平稳、波动
- **变化速率**：加速增长、减速增长、匀速下降等
- **周期性模式**：是否存在季节性、周期性、节假日效应？
- **拐点识别**：趋势发生逆转的关键时间节点
- **移动平均**：通过 N 期移动平均平滑短期波动，识别长期趋势

#### 4.3 归因分析（Why did it happen）

**这是报告丰富度的核心！** 深入分析数据变化的原因：
...
- **相关性分析**：
  - 不同指标之间是否存在关联？（如用户量与收入、活跃度与留存率）
  - 可能的因果链条推理（注意：相关不等于因果）

#### 4.4 对比分析（How does it compare）

通过多角度对比增加分析深度：

- **时间对比**：同比（去年同期）、环比（上月/上季度）、定基比（基准期）
- **分组对比**：不同地区、产品、渠道、客群之间的差异
- **目标对比**：实际值 vs 目标值、预算值、行业均值
- **排名对比**：Top N 和 Bottom N 分析，帕累托（80/20）分析
- **结构对比**：各组成部分的占比变化

#### 4.5 生成分析结论与建议

**⚠️ 报告必须包含结论部分！** 基于上述分析，生成结构化的结论：
...
**结论撰写要求：**
- 每个结论必须有**具体数字**支撑，不要泛泛而谈
- 使用对比来增强说服力（如"A地区增长23%，而B地区仅增长5%"）
- 区分**事实**（数据显示）和**推断**（可能的原因）
- 建议要具体可操作，不要过于笼统
...
**报告结构（必须包含所有部分）：**
- **报告标题** - 清晰描述报告内容和时间范围
- **执行摘要** - 用 2-3 句话概括报告核心发现，让读者快速了解要点
- **摘要统计卡片** - 顶部展示关键指标（KPI），如总计、平均值、最大值、增长率等，卡片需带同比/环比变化标识（↑↓）
...
- **详细数据表格** - 完整的数据列表，支持排序和筛选，关键数据行高亮
- **📊 分析结论与洞察** - **（必须包含，不可省略）** 包含：
  - 核心发现（带具体数字）
  - 归因分析（变化原因）
  - 风险提示（异常信号）
  - 可行建议（基于数据的建议）
...
**标准工作流程（必须遵循）：**
1. 查询数据，用 Markdown 向用户说明分析过程
2. 深度分析数据：描述性统计 → 趋势分析 → 归因分析 → 对比分析
3. 生成完整的 HTML 报告内容（必须包含"分析结论与洞察"卡片）
4. 使用 `<!-- REPORT_HTML_START -->` 和 `<!-- REPORT_HTML_END -->` 分隔符包裹 HTML 内容，直接输出到对话中
5. 在 HTML 输出后，简要总结报告核心发现和建议
    ...
    <div class="container">
        ...
        <div class="card" style="border-left: 4px solid var(--accent-color);">
            ...
            <div style="margin-bottom: 1.5rem;">
                <h3 style="font-size: 1.125rem; font-weight: 600; color: var(--accent-color); margin-bottom: 0.75rem;">
                    📊 核心发现
                </h3>
                <ul style="list-style: none; padding: 0;">
                    <li style="padding: 0.5rem 0; border-bottom: 1px solid var(--border-color);">
                        <strong>1.</strong> 总用户数达 15,234，其中华东地区以 5,234 用户（占比 34.4%）位居首位
                    </li>
                    <li style="padding: 0.5rem 0; border-bottom: 1px solid var(--border-color);">
                        <strong>2.</strong> 华东与华南合计贡献了 64.3% 的用户量，呈现明显的头部集中效应
                    </li>
                    <li style="padding: 0.5rem 0;">
                        <strong>3.</strong> 西北地区仅 110 用户（0.7%），与华东差距达 47.6 倍
                    </li>
                </ul>
            </div>

            <div style="margin-bottom: 1.5rem;">
                <h3 style="font-size: 1.125rem; font-weight: 600; color: #10b981; margin-bottom: 0.75rem;">
                    📈 趋势与归因
                </h3>
                <ul style="list-style: none; padding: 0;">
                    <li style="padding: 0.5rem 0; border-bottom: 1px solid var(--border-color);">
                        华东地区环比增长 12.3%，主要驱动因素：新用户注册量增加，贡献了整体增长的 45%
                    </li>
                    <li style="padding: 0.5rem 0; border-bottom: 1px solid var(--border-color);">
                        华北地区环比下降 3.2%，归因于活跃用户流失率上升（从 5% 升至 8%）
                    </li>
                    <li style="padding: 0.5rem 0;">
                        西南地区增长趋于停滞（0.5%），需关注用户获取渠道的有效性
                    </li>
                </ul>
            </div>

            <div style="margin-bottom: 1.5rem;">
                <h3 style="font-size: 1.125rem; font-weight: 600; color: #f59e0b; margin-bottom: 0.75rem;">
                    ⚠️ 风险提示
                </h3>
                <ul style="list-style: none; padding: 0;">
                    <li style="padding: 0.5rem 0; border-bottom: 1px solid var(--border-color);">
                        用户分布极度不均（基尼系数 0.72），过度依赖华东华南市场
                    </li>
                    <li style="padding: 0.5rem 0;">
                        华北地区连续 2 个月下降，若持续可能导致整体增速放缓
                    </li>
                </ul>
            </div>

            <div>
                <h3 style="font-size: 1.125rem; font-weight: 600; color: #8b5cf6; margin-bottom: 0.75rem;">
                    💡 建议
                </h3>
                <ul style="list-style: none; padding: 0;">
                    <li style="padding: 0.5rem 0; border-bottom: 1px solid var(--border-color);">
                        <strong>短期：</strong>针对华北地区启动用户召回计划，目标将流失率降至 5% 以下
                    </li>
                    <li style="padding: 0.5rem 0; border-bottom: 1px solid var(--border-color);">
                        <strong>中期：</strong>加大西南、东北地区的市场推广投入，目标用户占比提升至 15%
                    </li>
                    <li style="padding: 0.5rem 0;">
                        <strong>长期：</strong>建立区域差异化运营策略，降低对头部地区的依赖
                    </li>
                </ul>
            </div>
        </div>
...
### 模式 7：归因分析报告
"分析本月销售额下降的原因"
→ 查询本月 vs 上月各维度数据（地区、产品、渠道等）
→ 计算各维度的增量贡献：`(本月值 - 上月值)` 占总变化量的百分比
→ 识别拖累最大的维度和驱动增长的维度
→ 使用瀑布图或堆叠柱状图展示贡献度分解
→ 结论中明确指出：XX维度贡献了下降的YY%

### 模式 8：异常检测报告
"检测过去30天的异常数据"
→ 计算历史均值和标准差
→ 标记偏离均值超过 2σ 的数据点
→ 按时间排列异常事件，标注异常类型（突增/突降）
→ 使用折线图 + 异常标注（红色标记点）
→ 分析异常可能的原因（时间关联、事件关联）
